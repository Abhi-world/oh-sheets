...
Deno.serve(async (req) => {
  ...
  try {
    console.log('📋 [list-google-spreadsheets] Request received');
    
    let body = {};
    try { body = await req.json(); } catch { body = {}; }
    const params = new URL(req.url).searchParams;
    
    // Extract monday_user_id from various sources
    ...
    if (!monday_user_id) {
      console.warn('⚠️ No monday_user_id provided, using fallback...');
      const { data: fallbackProfiles } = await supabase
        .from('profiles').select('monday_user_id')
        .not('google_sheets_credentials', 'is', null)
        .order('updated_at', { ascending: false }).limit(1);
      if (!fallbackProfiles || fallbackProfiles.length === 0) {
        console.error('❌ No profiles found with Google Sheets credentials');
        return new Response(JSON.stringify({ error: 'Google Sheets not connected.', options: [] }), { status: 400, headers: corsHeaders });
      }
      monday_user_id = String(fallbackProfiles[0].monday_user_id);
      console.log('✅ Using fallback monday_user_id:', monday_user_id);
    }

    console.log('🔍 Fetching profile credentials...');
    ...
    console.log('📊 Fetching spreadsheets from Google Drive API...');
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: errorData.error || 'Failed to fetch spreadsheets.', options: [] }), { status: response.status, headers: corsHeaders });
    }
    ...
    return new Response(JSON.stringify({ error: error.message || 'Internal server error', options: [] }), { status: 500, headers: corsHeaders });
  }
});
